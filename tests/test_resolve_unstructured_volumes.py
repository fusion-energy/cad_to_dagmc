...
class TestResolveUnstructuredVolumes:
    """Test the resolve_unstructured_volumes helper function"""
    ...
    def test_str_only_single_match(self):
    ...
    def test_str_only_multiple_matches(self):
    ...
    def test_mixed_str_and_int(self):
    ...
    def test_duplicate_from_multiple_tags(self):
    ...
    def test_invalid_material_tag_raises_error(self):
    ...
    def test_error_message_shows_available_tags(self):
...
class TestUnstructuredVolumesWithMaterialTagsIntegration:
    """Integration tests for unstructured_volumes with material tag strings"""

    def test_unstructured_volumes_with_material_tag_string(self, tmp_path):
        """Test export_dagmc_h5m_file with material tag string in unstructured_volumes"""
        # Create geometry with multiple volumes
    ...
    def test_unstructured_volumes_with_mixed_int_and_string(self, tmp_path):
    ...
    def test_unstructured_volumes_invalid_material_tag_raises_error(self, tmp_path):
    ...
    def test_unstructured_volumes_with_assembly_names(self, tmp_path):
        """Test using assembly names as material tags and selecting by name in unstructured_volumes.

        This test creates two cubes, adds them to an assembly with names,
        uses assembly_names to derive material tags, then uses one of the
        names to specify unstructured_volumes.
        """
        # Create two cubes at different positions
        cube1 = cq.Workplane("XY").box(10, 10, 10)
        cube2 = cq.Workplane("XY").center(20, 0).box(10, 10, 10)

        # Add to assembly with names
        assembly = cq.Assembly()
        assembly.add(cube1, name="steel_block")
        assembly.add(cube2, name="copper_block")

        # Create model and use assembly_names to derive material tags
        model = CadToDagmc()
        ...
        # Use the assembly name to specify which volume to mesh
        dag_filename, umesh_filename = model.export_dagmc_h5m_file(
            filename=str(h5m_file),
            min_mesh_size=0.5,
            max_mesh_size=2.0,
