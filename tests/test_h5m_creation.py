...
def test_cq_compound():
    ...
    my_model.export_dagmc_h5m_file(filename='compound_dagmc.h5m', max_mesh_size=0.2, min_mesh_size=0.1)

    assert Path('compound_dagmc.h5m').is_file()
    assert di.get_volumes_and_materials_from_h5m('compound_dagmc.h5m') == {1: "mat1", 2: "mat2"}
