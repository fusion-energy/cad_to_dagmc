...
def test_cadquery_assembly_with_materials():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assembly.add(result, name='result', material=cq.Material('diamond'))  # note material assigned here
        assembly.add(result2, name='result2', material=cq.Material('gold'))  # note material assigned here

        my_model = CadToDagmc()
        my_model.add_cadquery_object(cadquery_object=assembly)  # note that material tags are not needed here
